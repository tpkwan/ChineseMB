...
/**
 * This Fragment Contains All Method Related To JomEventGroupFragment.
 * 
 * @author tasol
 * 
 */
@SuppressLint("ValidFragment")
public class JomEventGroupFragment extends SmartFragment implements JomTagHolder {
	...
	private JomEventDataProvider providerGroupEvent;
	private AQuery androidQuery;
	
	private String IN_USERID;
	private String IN_GROUP_ID;

	public JomEventGroupFragment() {
	}
	...
	public int setLayoutId() {
		return R.layout.jom_event_group_fragment;
	}
	...
	public void initComponents(View currentView) {
		...
		providerGroupEvent = new JomEventDataProvider(getActivity());
		...
		lstEvent.setOnScrollListener(new OnScrollListener() {
			...
			public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
				...
				if ((firstVisibleItem + visibleItemCount) >= totalItemCount && totalItemCount > 1) {
					if (!providerGroupEvent.isCalling() && providerGroupEvent.hasNextPage()) {
						listFooterVisible();
						providerGroupEvent.getGroupEventList(IN_GROUP_ID, new WebCallListener() {
							...
							public void onCallComplete(int responseCode, String errorMessage, ArrayList<HashMap<String, String>> data1, Object data2) {
								try {
									listFooterInvisible();
									if (responseCode == 200) {
										((JomMasterActivity) getActivity()).updateHeader(providerGroupEvent.getNotificationData());
	...
	/**
	 * This method used to prepare list group event.
