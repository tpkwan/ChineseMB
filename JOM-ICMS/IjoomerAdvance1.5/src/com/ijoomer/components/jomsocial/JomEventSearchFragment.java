...
/**
 * This Fragment Contains All Method Related To JomEventSearchFragment.
 * 
 * @author tasol
 * 
 */
@SuppressLint("ValidFragment")
public class JomEventSearchFragment extends SmartFragment implements JomTagHolder {
	...
	public int setLayoutId() {
		return R.layout.jom_event_search_fragment;
	}
	...
	public void initComponents(View currentView) {
		...
		providerSearchEvent = new JomEventDataProvider(getActivity());
	...
	public void prepareViews(View currentView) {
		if (adapterEvent == null) {
			getSeachEventList(true);
		} else {
			lstEvent.setAdapter(adapterEvent);
			getSeachEventList(false);
		}
			...
			public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
				...
				if ((firstVisibleItem + visibleItemCount) >= totalItemCount && totalItemCount > 1) {
					if (!providerSearchEvent.isCalling() && providerSearchEvent.hasNextPage()) {
						listFooterVisible();
						providerSearchEvent.searchEventList(null, serachKeyword, null, null, null, null, "latest", new WebCallListener() {
							...
							public void onCallComplete(int responseCode, String errorMessage, ArrayList<HashMap<String, String>> data1, Object data2) {
								try {
									listFooterInvisible();
									if (responseCode == 200) {
										((JomMasterActivity) getActivity()).updateHeader(providerSearchEvent.getNotificationData());
	...
	/**
	 * This method used to get search event.
	 * @param isProgressShow represented progress shown
	 */
	private void getSeachEventList(final boolean isProgressShow) {
	...
	/**
	 * This method used to prepare list search event.
