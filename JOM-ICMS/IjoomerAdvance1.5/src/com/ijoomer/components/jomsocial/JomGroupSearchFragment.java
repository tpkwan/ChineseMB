...
/**
 * This Fragment Contains All Method Related To JomGroupSearchFragment.
 * 
 * @author tasol
 * 
 */
@SuppressLint("ValidFragment")
public class JomGroupSearchFragment extends SmartFragment implements JomTagHolder {
	...
	public int setLayoutId() {
		return R.layout.jom_group_search_fragment;
	}
	...
	public void initComponents(View currentView) {
		...
		providerSearchGroup = new JomGroupDataProvider(getActivity());
			...
			public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
				...
				if ((firstVisibleItem + visibleItemCount) >= totalItemCount && totalItemCount > 1) {
					if (!providerSearchGroup.isCalling() && providerSearchGroup.hasNextPage()) {
						listFooterVisible();
						providerSearchGroup.searchGroupList(null, serachKeyword, null, new WebCallListener() {
							...
							public void onCallComplete(int responseCode, String errorMessage, ArrayList<HashMap<String, String>> data1, Object data2) {
								try {
									listFooterInvisible();
									if (responseCode == 200) {
										((JomMasterActivity) getActivity()).updateHeader(providerSearchGroup.getNotificationData());
	...
	/**
	 * This method used to get search group.
	 * @param isProgressShow represented progress shown
	 */
	private void getSearchGroup(final boolean isProgressShow) {
	...
	private void responseErrorMessageHandler(final int responseCode, final boolean finishActivityOnConnectionProblem) {
		IjoomerUtilities.getCustomOkDialog(getString(R.string.group), getString(getResources().getIdentifier("code" + responseCode, "string", getActivity().getPackageName())), getString(R.string.ok), R.layout.ijoomer_ok_dialog, new CustomAlertNeutral() {
	...
	/**
	 * This method used to prepare list search group.
