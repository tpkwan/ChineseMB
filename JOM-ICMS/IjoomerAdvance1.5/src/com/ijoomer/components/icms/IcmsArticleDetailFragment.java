	...
	public void prepareArticleDetail(ArrayList<HashMap<String, String>> data) {
		...
		if (data != null) {
			...
			try {
				...
				if (data.get(0).get(AUTHORALIAS) != null && data.get(0).get(AUTHORALIAS).trim().length() > 0) {
					txtWrittenBy.setText(getString(R.string.writtenby) + " " + data.get(0).get(AUTHORALIAS));
				} else if (data.get(0).get(CRETEDBY) != null && data.get(0).get(CRETEDBY).trim().length() > 0) {
