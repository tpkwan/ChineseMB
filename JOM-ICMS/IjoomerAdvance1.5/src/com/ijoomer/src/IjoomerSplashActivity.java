	...
	private void authentication() {
		globalConfiguration.loadGlobalConfiguration(new WebCallListener() {
			...
			public void onCallComplete(int responseCode, String errorMessage,
				...
				if (responseCode == 200) {
					if (IjoomerGlobalConfiguration.isLoginRequired()
							|| IjoomerUtilities.getLoginParams() != null
							&& IjoomerUtilities.getLoginParams().toString()
									.length() > 0) {
						if (!getSmartApplication().readSharedPreferences()
								.getBoolean(SP_ISLOGOUT, true)) {
							IjoomerOauth
									.getInstance(IjoomerSplashActivity.this)
									.autoLogin(
											IjoomerUtilities.getLoginParams()
													.toString(),
											new WebCallListener() {
												...
												public void onCallComplete(
													...
													if (responseCode == 200) {
														globalConfiguration
																.loadGlobalConfiguration(new WebCallListener() {
						...
						if (getSmartApplication().readSharedPreferences()
								.getString(SP_DEFAULT_LANDING_SCREEN, "")
								.length() > 0) {
							try {
								Intent intent = new Intent();
								intent.setClassName(
