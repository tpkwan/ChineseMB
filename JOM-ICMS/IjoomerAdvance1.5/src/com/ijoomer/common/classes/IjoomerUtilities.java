...
public class IjoomerUtilities implements IjoomerSharedPreferences {
	 ...
	 *            click action.
	 */
	public static void getOKDialog(final String title, final String msg,
			final String IjoomerButtonCaption, final boolean isCancelable,
			final AlertNeutral target) {
		if (!msg.equalsIgnoreCase(mSmartAndroidActivity
				.getString(R.string.code704))) {
				...
				mSmartAndroidActivity.runOnUiThread(new Runnable() {
					...
					public void run() {
						AlertDialog.Builder builder = new AlertDialog.Builder(
								mSmartAndroidActivity);

						int imageResource = android.R.drawable.stat_sys_warning;
						Drawable image = mSmartAndroidActivity.getResources()
						...
						builder.setTitle(title)
								...
								.setIcon(image)
								.setCancelable(false)
								.setNeutralButton(IjoomerButtonCaption,
	 ...
	 */
	public static void getCustomOkDialog(final String title, final String msg,
			final String IjoomerButtonCaption, final int layoutID,
			final CustomAlertNeutral target) {
		if (!msg.equals(mSmartAndroidActivity.getResources().getString(
				R.string.code704))) {
			mSmartAndroidActivity.runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					final Dialog dialog = new Dialog(mSmartAndroidActivity,
							android.R.style.Theme_Translucent_NoTitleBar);
					dialog.setContentView(ThemeManager.getInstance()
							.getOkDialog());

					IjoomerTextView txtTitle = (IjoomerTextView) dialog
							.findViewById(R.id.txtTitle);
					IjoomerTextView txtMessage = (IjoomerTextView) dialog
							.findViewById(R.id.txtMessage);
					txtMessage.setMovementMethod(LinkMovementMethod
							.getInstance());
					txtMessage.setText(Html.fromHtml(msg));
					txtTitle.setText(title);
					IjoomerButton ok = (IjoomerButton) dialog
	...
	public static void getReportDialog(final ReportListner target) {
		...
		final Spinner spnReportType = (Spinner) dialog
				.findViewById(R.id.spnReportType);
		final IjoomerEditText edtReportMessage = (IjoomerEditText) dialog
				.findViewById(R.id.edtReportMessage);
		IjoomerButton btnCancel = (IjoomerButton) dialog
				.findViewById(R.id.btnCancel);
		IjoomerButton btnSend = (IjoomerButton) dialog
				.findViewById(R.id.btnSend);

		spnReportType.setAdapter(new MyCustomAdapter(mSmartAndroidActivity,
				new ArrayList<String>(Arrays.asList(mSmartAndroidActivity
		...
		btnSend.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View paramView) {
				target.onClick(spnReportType.getSelectedItem().toString(),
						edtReportMessage.getText().toString().trim());
				dialog.dismiss();
			}
		...
		mSmartAndroidActivity.runOnUiThread(new Runnable() {
			...
			public void run() {
				try {
					...
					dialog.setContentView(ThemeManager.getInstance()
							.getLoadingDialog());
					final IjoomerTextView txtMessage = (IjoomerTextView) dialog
							.findViewById(R.id.txtMessage);
					final IjoomerTextView txtProgrss = (IjoomerTextView) dialog
							.findViewById(R.id.txtProgrss);
		...
		mSmartAndroidActivity.runOnUiThread(new Runnable() {
			...
			public void run() {
				try {
					...
					final IjoomerTextView txtProgrss = (IjoomerTextView) dialog
							.findViewById(R.id.txtProgrss);
					final IjoomerTextView txtSize = (IjoomerTextView) dialog
							.findViewById(R.id.txtSize);
					txtSize.setVisibility(View.VISIBLE);
					txtSize.setText(mSmartAndroidActivity
	 ...
	 *            represented {@link CustomAlertMagnatic}
	 */
	public static void getCustomConfirmDialog(final String title,
			final String msg, final String okIjoomerButtonCaption,
			final String cancelIjoomerButtonCaption,
	 ...
	 */
	public static void getContactDialog(
			final ArrayList<HashMap<String, Object>> selected,
			final ShareListner target) {

		int popupWidth = ((SmartActivity) mSmartAndroidActivity)
				.getDeviceWidth()
				- ((SmartActivity) mSmartAndroidActivity)
						.convertSizeToDeviceDependent(50);
		int popupHeight = ((SmartActivity) mSmartAndroidActivity)
				.getDeviceHeight()
				- ((SmartActivity) mSmartAndroidActivity)
						.convertSizeToDeviceDependent(130);

		LayoutInflater layoutInflater = (LayoutInflater) mSmartAndroidActivity
				.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		View layout = layoutInflater.inflate(ThemeManager.getInstance()
				.getContactDilaog(), null);
		...
		popup.setBackgroundDrawable(new BitmapDrawable(mSmartAndroidActivity
				.getResources()));
		popup.showAtLocation(layout, Gravity.CENTER_HORIZONTAL
				| Gravity.CENTER_VERTICAL, 0, 0);

		final ListView lstContact = (ListView) layout
				.findViewById(R.id.lstContact);
		IjoomerButton btnCancel = (IjoomerButton) layout
				.findViewById(R.id.btnCancel);
		final IjoomerRadioButton rdbSelectAll = (IjoomerRadioButton) layout
				.findViewById(R.id.rdbSelectAll);
		final IjoomerRadioButton rdbSelectNone = (IjoomerRadioButton) layout
				.findViewById(R.id.rdbSelectNone);
		rdbSelectAll.setVisibility(View.GONE);
		rdbSelectNone.setVisibility(View.GONE);
		IjoomerButton btnDone = (IjoomerButton) layout
		...
		if (selected != null && selected.size() > 0) {
			...
			if (prepareList(getContacts()).size() > 0) {
				...
				IjoomerUtilities.getCustomOkDialog(mSmartAndroidActivity
						.getString(R.string.dialog_contact),
						mSmartAndroidActivity
								.getString(R.string.dialog_contact_not_found),
						mSmartAndroidActivity.getString(R.string.ok),
						ThemeManager.getInstance().getOkDialog(),
		...
		btnDone.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				target.onClick("Email", newSelectedList, selectedString != null
						&& selectedString.trim().length() > 0 ? selectedString
			...
			public void onClick(View arg0) {
				if (contactAdapter != null) {
					...
					for (int i = 0; i < size; i++) {
						((HashMap<String, String>) (contactAdapter.getItem(i))
								.getValues().get(0)).put("isChecked", "true");
					}
		...
		rdbSelectNone.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (contactAdapter != null) {
					...
					for (int i = 0; i < size; i++) {
						((HashMap<String, String>) (contactAdapter.getItem(i))
								.getValues().get(0)).put("isChecked", "false");
					}
					contactAdapter.notifyDataSetChanged();
				}
			}
	 ...
	 */
	private static SmartListAdapterWithHolder getListAdapter(
			final ArrayList<SmartListItem> data) {

		SmartListAdapterWithHolder adapterWithHolder = new SmartListAdapterWithHolder(
				mSmartAndroidActivity, ThemeManager.getInstance()
						.getContactItemDilaog(), data, new ItemView() {
					@SuppressWarnings("unchecked")
					@Override
					public View setItemView(final int position, View v,
							final SmartListItem item, final ViewHolder holder) {

						holder.txtContactName = (IjoomerTextView) v
								.findViewById(R.id.txtContactName);
						holder.imgContactUser = (ImageView) v
								.findViewById(R.id.imgContactUser);
						holder.txtContactEmail = (IjoomerTextView) v
								.findViewById(R.id.txtContactEmail);
						holder.chbContact = (IjoomerCheckBox) v
								.findViewById(R.id.chbContact);

						final HashMap<String, Object> row = (HashMap<String, Object>) item
								.getValues().get(0);
						holder.chbContact
						...
						try {
							holder.imgContactUser.setImageBitmap((Bitmap) (row)
									.get("photo"));
						} catch (Throwable e) {
							holder.imgContactUser
									.setImageResource(R.drawable.ic_launcher);
						}
						holder.txtContactName.setText((row).get("name")
								.toString());
						String[] emailArray = (row).get("email").toString()
								.split(";");
					...
					@Override
					public View setItemView(int position, View v,
							SmartListItem item) {
						return null;
					}
				});
	 ...
	 */
	private static ArrayList<SmartListItem> prepareList(
		...
		if (data != null) {
			...
			for (int i = 0; i < size; i++) {
				...
				item.setItemLayout(ThemeManager.getInstance()
	 ...
	 *            click action.
	 */
	public static void getConfirmDialog(String title, String msg,
		...
		Drawable image = mSmartAndroidActivity.getResources().getDrawable(
				imageResource);
	...
	public static boolean isNetwokReachable() {
		final ConnectivityManager connMgr = (ConnectivityManager) mSmartAndroidActivity
	 ...
	 */
	public static void getMultiSelectionDialog(final String name,
			String jsonString, final String value,
			final CustomClickListner target) {
		...
		if (value.length() > 0) {
			...
			for (int i = 0; i < size; i++) {
				...
				for (int j = 0; j < len; j++) {
					if (values.get(i).toString().trim()
							.equalsIgnoreCase(oldValue[j].trim())) {
						selections[i] = true;
						break;
					}
				}
		...
		builder.setMultiChoiceItems(
				values.toArray(new CharSequence[values.size()]), selections,
		...
		builder.setPositiveButton(mSmartAndroidActivity.getString(R.string.ok),
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						for (int i = 0; i < size; i++) {
							if (selections[i]) {
								newValue.append(newValue.length() > 0 ? ","
		...
		builder.setNegativeButton(
				mSmartAndroidActivity.getString(R.string.cancel),
				new DialogInterface.OnClickListener() {
	...
	public static void getMultiSelectionDialogSobipro(final String name,
			String jsonString, final String value, final String id,
		...
		try {
			...
			int size = jsonArray.length();
			for (int i = 0; i < size; i++) {
				values.add(((JSONObject) jsonArray.get(i)).getString("name")
						.trim());
				ids.add(((JSONObject) jsonArray.get(i)).getString("value")
						.trim());
			}
	 ...
	 *            represented {@link CustomClickListner}
	 */
	public static void getDateDialog(final String strDate, boolean restrict,
			final CustomClickListner target) {
		...
		IjoomerDataPickerView dateDlg = new IjoomerDataPickerView(
				mSmartAndroidActivity,
	 ...
	 *            represented {@link CustomClickListner}
	 */
	public static void getDateTimeDialog(final String strDate,
			final CustomClickListner target) {
		...
		DatePickerDialog dateDialog = new DatePickerDialog(
				...
				new DatePickerDialog.OnDateSetListener() {
					public void onDateSet(DatePicker view, int year,
						...
						new TimePickerDialog(mSmartAndroidActivity,
								new TimePickerDialog.OnTimeSetListener() {
									...
									public void onTimeSet(TimePicker view,
										...
										Time chosenDate = new Time();
										chosenDate.set(0, minute, hourOfDay, d,
												m, y);
										long dt = chosenDate.toMillis(true);
										CharSequence strDate = DateFormat
												.format(IjoomerApplicationConfiguration.dateTimeFormat,
														dt);
										target.onClick(strDate.toString());
									}
								}, date.get(Calendar.HOUR), date
	 ...
	 *            represented {@link CustomClickListner}
	 */
	public static void getTimeDialog(final String strTime,
			final CustomClickListner target) {
				...
				new TimePickerDialog.OnTimeSetListener() {
					...
					public void onTimeSet(TimePicker view, int hourOfDay,
						...
						String dateString = new SimpleDateFormat(
								IjoomerApplicationConfiguration.timeFormat)
								.format(date);
	...
	public static MyCustomAdapter getSpinnerAdapter(
		...
		try {
			...
			for (int i = 0; i < size; i++) {
				...
				if (options.getString("value").equals(field.get("value"))
						|| options.has("title")
						&& options.getString("title")
								.equals(field.get("value"))
						|| options.has("name")
		...
		final MyCustomAdapter adpater = new MyCustomAdapter(
				mSmartAndroidActivity, values);
	 ...
	 * @return represented {@link MyCustomAdapter}
	 */
	public static MyCustomAdapter getPrivacySpinnerAdapter(
			HashMap<String, String> field) {
		...
		try {
			jsonArray = new JSONObject(field.get("privacy"))
			...
			for (int i = 0; i < size; i++) {
				...
				if (options.getString("value")
						.equals(new JSONObject(field.get("privacy"))
								.getString("value"))) {
					index = i;
				}
			}
		...
		MyCustomAdapter adpater = new MyCustomAdapter(mSmartAndroidActivity,
				values);
		adpater.setDefaultPostion(index);
		return adpater;
	...
	public static boolean birthdateValidator(String birthDate) {
		SimpleDateFormat dateFormat = new SimpleDateFormat(
				IjoomerApplicationConfiguration.dateFormat);
		try {
	...
	public static void selectImageDialog(final SelectImageDialogListner target) {
		...
		final IjoomerTextView txtCapture = (IjoomerTextView) dialog
				.findViewById(R.id.txtCapture);
		final IjoomerTextView txtPhoneGallery = (IjoomerTextView) dialog
				.findViewById(R.id.txtPhoneGallery);
	 ...
	 */
	public static ArrayList<HashMap<String, String>> getListIndexedData(
			ArrayList<HashMap<String, String>> oldData, String indexOn) {
	...
	public static Address getAddressFromLatLong(double lat, double lng) {
		...
		if (lat == 0 || lng == 0) {
			lat = Double.parseDouble(((SmartActivity) mSmartAndroidActivity)
					.getLatitude());
			lng = Double.parseDouble(((SmartActivity) mSmartAndroidActivity)
					.getLongitude());
		}
		geocoder = new Geocoder(mSmartAndroidActivity);
	 ...
	 *            represented {@link CustomClickListner}
	 */
	public static void getFilteringMapAddressDialog(final double lat,
			final double lng, final CustomClickListner target) {
		mSmartAndroidActivity.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				ListView lstMapAddress = (ListView) dialog
						.findViewById(R.id.lstMapAddress);
				ImageView imgContactClose = (ImageView) dialog
						.findViewById(R.id.imgContactClose);

				adapter = getMapAddressListAdapter(prepareListMapAddress(getAddressListFromLatLong(
						lat, lng)));
				...
				lstMapAddress.setOnItemClickListener(new OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> arg0, View arg1,
							int pos, long arg3) {
						Address address = (Address) adapter.getItem(pos)
								.getValues().get(0);
						target.onClick(address.getAddressLine(0) + ","
	 ...
	 * @return represented {@link SmartListAdapterWithHolder}
	 */
	private static SmartListAdapterWithHolder getMapAddressListAdapter(
			final ArrayList<SmartListItem> data) {

		SmartListAdapterWithHolder adapterWithHolder = new SmartListAdapterWithHolder(
				mSmartAndroidActivity,
				R.layout.ijoomer_filtering_map_dialog_item, data,
				new ItemView() {
					@Override
					public View setItemView(final int position, View v,
							final SmartListItem item, final ViewHolder holder) {

						holder.txtMapAddress = (IjoomerTextView) v
								.findViewById(R.id.txtMapAddress);

						final Address row = (Address) item.getValues().get(0);
						holder.txtMapAddress.setText(row.getAddressLine(0)
								+ "," + row.getAddressLine(1) + ","
								+ row.getAddressLine(2));
						return v;
					}
	 ...
	 * @return represented {@link SmartListItem} list
	 */
	private static ArrayList<SmartListItem> prepareListMapAddress(
	...
	public static ArrayList<HashMap<String, Object>> getContacts() {
		...
		final Cursor rawContacts = mSmartAndroidActivity.managedQuery(
				RawContacts.CONTENT_URI, projection, null, null, null);
	...
	private static String getName(int contactId) {
		...
		final Cursor contact = mSmartAndroidActivity.managedQuery(
	...
	public String getReportCode(String privacy) {
		...
		ArrayList<String> list = new ArrayList<String>(
				Arrays.asList(mSmartAndroidActivity.getResources()
	 ...
	 */
	public static void IjoomerTextViewResizable(final IjoomerTextView tv,
		...
		vto.addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
			...
			public void onGlobalLayout() {
				...
				if (maxLine <= 0) {
					...
					tv.setText(
							addClickablePartIjoomerTextViewResizable(
									Html.fromHtml(tv.getText().toString()), tv,
									maxLine, expandText), BufferType.SPANNABLE);
	 ...
	 */
	private static SpannableStringBuilder addClickablePartIjoomerTextViewResizable(
			final Spanned strSpanned, final IjoomerTextView tv,
			final int maxLine, final String expandText) {
		...
		if (str.contains(expandText)) {
			ssb.setSpan(
					new IjoomerSpannable(Color.parseColor(mSmartAndroidActivity
							.getString(R.color.blue)), true) {
					...
					}, str.indexOf(expandText), str.indexOf(expandText)
							+ expandText.length(), 0);
	...
	public static String getDateStringCurrentTimeZone(long timestamp) {
		...
		TimeZone t = TimeZone.getTimeZone(IjoomerGlobalConfiguration
				.getServerTimeZone());
		...
		SimpleDateFormat sdf = new SimpleDateFormat(
				IjoomerApplicationConfiguration.dateTimeFormat);
		...
		if (diff[0] > 0) {
			...
			if (c.getMaximum(Calendar.DATE) <= diff[0]) {
				return (String) DateFormat.format(
						IjoomerApplicationConfiguration.dateFormat, start);
			} else {
				return diff[0] > 1 ? String.format("%d days ago", diff[0])
						: String.format("%d day ago", diff[0]);
			}
		} else if (diff[1] > 0) {
			return diff[1] > 1 ? String.format("%d hours ago", diff[1])
					: String.format("%d hour ago", diff[1]);
		} else if (diff[2] > 0) {
			return diff[2] > 1 ? String.format("%d minutes ago", diff[2])
					: String.format("%d minute ago", diff[2]);
		} else if (diff[3] > 0) {
			return diff[3] > 1 ? String.format("%d seconds ago", diff[3])
					: String.format("%d second ago", diff[3]);
		} else {
			return (String) DateFormat.format(
					IjoomerApplicationConfiguration.dateFormat, start);
		}
	...
	public static JSONObject getLoginParams() {
		...
		try {
			...
			taskData.put("lat",
					((SmartActivity) mSmartAndroidActivity).getLatitude());
			taskData.put("long",
					((SmartActivity) mSmartAndroidActivity).getLongitude());
	 ...
	 */
	public static void getTermsNConditionDialog(final String title,
			final String termsNCondition) {
		final Dialog dialog = new Dialog(mSmartAndroidActivity,
				android.R.style.Theme_Translucent_NoTitleBar);
		dialog.setContentView(R.layout.ijoomer_terms_n_condition_dialog);
		final IjoomerTextView txtTermsNConditionTitle = (IjoomerTextView) dialog
				.findViewById(R.id.txtTermsNConditionTitle);
		final WebView webTermsNCondition = (WebView) dialog
				.findViewById(R.id.webTermsNCondition);
		final ImageView imgTermsNConditionClose = (ImageView) dialog
				.findViewById(R.id.imgTermsNConditionClose);

		txtTermsNConditionTitle.setText(title);
		webTermsNCondition.loadDataWithBaseURL("file:///android_asset/css/",
				termsNCondition, "text/html", "utf-8", null);
	...
	public static void addToNotificationBar(String ticker, String title,
		...
		PendingIntent contentIntent = PendingIntent.getActivity(
				mSmartAndroidActivity, 0, new Intent(), 0);
		Notification notification = new Notification(icon, ticker, when);
		notification.setLatestEventInfo(mSmartAndroidActivity, title, message,
				contentIntent);
	 ...
	 *            represented {@link CustomAlertMagnatic}
	 */
	public static void getCustomOkCancelDialog(final String title,
			final String msg, final String IjoomerButtonCaption,
			final String IjoomerButtonCancelCaption, final int layoutID,
			final CustomAlertNeutral target) {
		if (!msg.equals(mSmartAndroidActivity.getResources().getString(
				R.string.code704))) {
			mSmartAndroidActivity.runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					IjoomerButton cancel = (IjoomerButton) dialog
							.findViewById(R.id.btnCancel);
	...
	public static double convertDistance(String valueToConvert, int fromUnit,
			int toUnit) {
		...
		if (dvalueToConvert > 0) {
			...
			if (toUnit == IjoomerUtilities.KILOMETER) {
				...
				dvalueToConvert = Math
						.acos(((dvalueToConvert / 1000d) / 6371d));
			}
